...
TEST(s8init, InitDefault)
{
    ...
    const s8    str_empty = s8_init(allocated_len, &libc_malloc_ctx);
    ...
    s8_free(str_empty, &libc_malloc_ctx);
}
...
TEST(s8from, FromStringLargerThanBuffer)
{
    ...
    const s8 poem_truncated = s8_from(allocated_len, FROST_POEM, &libc_malloc_ctx);
    ...
    s8_free(poem_truncated, &libc_malloc_ctx);
}

TEST(s8from, FromStringEquallySizedToBuffer)
{
    const usize allocated_len = lengthof(FROST_POEM);
    const s8    poem_snug = s8_from(allocated_len, FROST_POEM, &libc_malloc_ctx);
    ...
    s8_free(poem_snug, &libc_malloc_ctx);
}

TEST(s8from, FromStringSmallerThanBuffer)
{
    const usize allocated_len = sizeof(FROST_POEM) + 32;
    const s8    poem_extraneous = s8_from(allocated_len, FROST_POEM, &libc_malloc_ctx);
    EXPECT_EQ(s8_capacity(poem_extraneous), allocated_len);
    EXPECT_EQ(memcmp(poem_extraneous, FROST_POEM, sizeof(FROST_POEM)), 0);
    s8_free(poem_extraneous, &libc_malloc_ctx);
}

TEST(s8clone, String)
{
    const s8 poem = S8(FROST_POEM, &libc_malloc_ctx);
    const s8 forgery = s8_clone(poem, &libc_malloc_ctx);
    EXPECT_EQ(memcmp(poem, forgery, sizeof(FROST_POEM)), 0);
    EXPECT_EQ(s8_capacity(poem), s8_capacity(forgery));
    EXPECT_NE(poem, forgery);
    s8_free(forgery, &libc_malloc_ctx);
    s8_free(poem, &libc_malloc_ctx);
}

TEST(s8clone, EmptyString)
{
    const s8 nothing = S8("", &libc_malloc_ctx);
    const s8 empty = s8_clone(nothing, &libc_malloc_ctx);
    EXPECT_EQ(nothing[0], 0);
    EXPECT_EQ(s8_capacity(nothing), 0);
    s8_free(empty, &libc_malloc_ctx);
    s8_free(nothing, &libc_malloc_ctx);
}
...
TEST(s8free, FreedMemoryZeroed)
{
    ...
    TomatoAllocCtx zero_checker{wrapped_malloc, checked_free, nullptr};
...
{
    s8 poem = S8(FROST_POEM, &libc_malloc_ctx);
...
TEST(s8cmp, Strings)
{
    s8 poem_frost = S8(FROST_POEM, &libc_malloc_ctx);
    s8 poem_gilpin = S8(GILPIN_POEM, &libc_malloc_ctx);
    EXPECT_LT(s8_cmp(poem_frost, poem_gilpin), 0);
    EXPECT_GT(s8_cmp(poem_gilpin, poem_frost), 0);
    s8_free(poem_gilpin, &libc_malloc_ctx);
    s8_free(poem_frost, &libc_malloc_ctx);
}

TEST(s8cmp, SameLengthStrings)
{
    s8 poem_frost = s8_from(64, FROST_POEM, &libc_malloc_ctx);
    s8 poem_gilpin = s8_from(64, GILPIN_POEM, &libc_malloc_ctx);
    EXPECT_LT(s8_cmp(poem_frost, poem_gilpin), 0);
    EXPECT_GT(s8_cmp(poem_gilpin, poem_frost), 0);
    s8_free(poem_gilpin, &libc_malloc_ctx);
    s8_free(poem_frost, &libc_malloc_ctx);
...
{
    s8 poem = S8(FROST_POEM, &libc_malloc_ctx);
    s8 poem_truncated = s8_from(64, FROST_POEM, &libc_malloc_ctx);
...
{
    const usize len = 64;
    const s8    str = s8_init(len, &libc_malloc_ctx);
    const char  data[len]{};
    s8_fill(str, 0xFF);
    EXPECT_NE(memcmp(str, data, len), 0);
    s8_fill(str, 0x00);
    EXPECT_EQ(memcmp(str, data, len), 0);
    s8_free(str, &libc_malloc_ctx);
}